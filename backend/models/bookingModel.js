...
bookingModel.addOne = async (customerId, addressId, inventoryId, dropoff, pickup, bookingStatus, notes) => {

    let client;
    try{
        client = await pool.connect();
        const queryText = "INSERT INTO Booking (customerId, addressId, inventoryId, dropoff, pickup, bookingStatus, notes) VALUES ($1,$2,$3,$4,$5,$6, $7)";
        const queryParams = [customerId, addressId, inventoryId, dropoff, pickup, bookingStatus, notes];
...
bookingModel.updateOne = async (id, newInfo) => {
    ...
    try{
        ...
        const {inventoryid, customerid, addressid,dropoff , pickup, bookingstatus, notes} = updatedInfo;

        
        const queryText = "UPDATE Booking SET inventoryId = $1, customerId = $2, addressId = $3, dropoff = $4, pickup = $5, bookingStatus = $6, notes = $7 WHERE id = $8";
        const queryParams = [inventoryid, customerid, addressid,dropoff , pickup, bookingstatus,notes, id];
        await client.query(queryText,queryParams);
        return "success";
